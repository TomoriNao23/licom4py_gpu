...
def validate_time_params(cls):
    ...
    def check_time_params(self) -> None:
        if getattr(self, "start_month") is not None:
            self._require(
                1 <= self.start_month <= 12, "Start month must be between 1 and 12"
            )
        if getattr(self, "start_day") is not None:
            self._require(
                1 <= self.start_day <= 31, "Start day must be between 1 and 31"
            )
        if getattr(self, "start_hour") is not None:
            self._require(
                0 <= self.start_hour <= 23, "Start hour must be between 0 and 23"
            )
        if getattr(self, "start_minute") is not None:
            self._require(
                0 <= self.start_minute <= 59, "Start minute must be between 0 and 59"
            )
        if getattr(self, "start_second") is not None:
            self._require(
                0 <= self.start_second <= 59, "Start second must be between 0 and 59"
            )
        ...
        self._require(
            integration_specified,
            "At least one integration time component must be specified",
        )
...
def validate_namelist_class(cls):
    ...
    def check_namelist(self) -> None:
        ...
        self._require(
            self.baroclinic_dt > 0, "baroclinic_dt must be greater than zero."
        )
        self._require(
            self._total_integration_seconds >= self.baroclinic_dt,
            "total_integration_seconds must be at least as large as baroclinic_dt.",
        )
        self._require(
            self._total_integration_seconds % self.baroclinic_dt == 0,
            "total_integration_seconds must be divisible by baroclinic_dt.",
        )
        self._require(
            self.barotropic_dt > 0, "barotropic_dt must be greater than zero."
        )
        self._require(
            self.baroclinic_dt % self.barotropic_dt == 0,
            "baroclinic_dt must be an integer multiple of barotropic_dt.",
        )
        self._require(
            self.tracer_interval is None or self.tracer_interval > 0,
            "Tracer frequency must be a positive integer or None",
        )
        self._require(
            self.barotropic_dt <= self.baroclinic_dt,
